    ...
    switch(method) {
        ...
        try{
            ...
            if(response.data.length === 0  || response.data.quantity === 0){
                res.status(204).json({message: "No product was found or product has finished"})
            }else {
                // Check the user 
                const user  = await User.findOne({email: email}).exec()
                if(!user) return res.status(204).json({message: `No User matches this email ${email}`})
            // Add item to Cart
            if(size) user.cart.size = size;
            if(productId) user.cart.itemId = productId;
            if(quantity) user.cart.quantity = quantity;
